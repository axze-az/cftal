...
static INLINE vdouble2 vabs_vd2_vd2(vdouble2 x) {
  return vcast_vd2_vd_vd((vdouble)vxor_vm_vm_vm(vand_vm_vm_vm((vmask)vcast_vd_d(-0.0), (vmask)x.x), (vmask)x.x),
			 (vdouble)vxor_vm_vm_vm(vand_vm_vm_vm((vmask)vcast_vd_d(-0.0), (vmask)x.x), (vmask)x.y));
}
