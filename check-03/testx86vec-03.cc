...
namespace cftal {
    ...
    namespace test {
        ...
        template <class _V>
        struct func_data {
            typedef typename _V::value_type fp_type;
...
template <class _T>
_T cftal::test::rel_error(const _T& a, const _T& b)
{
    _T ae(a - b);
    _T r((a + b) * _T(0.5));
    _T re(ae / r);
...
int cftal::test::ulp(double c, double n)
{
...
std::string cftal::test::delete_comment(const std::string& s)
{
...
template <class _V>
bool cftal::test::read_func(func_data<_V>& tf,
                             std::istream& is,
                             bool use_native)
{
    std::string line;
    while (!getline(is, line).eof()) {
        ...
        if (f == "cos") {
            if (use_native)
                tf._f1 = cftal::native_cos;
            else
                tf._f1 = cftal::cos;
            tf._f1d= std::cos;
        } else if (f == "sin") {
            if (use_native)
                tf._f1 = cftal::native_sin;
            else
                tf._f1 = cftal::sin;
            tf._f1d= std::sin;
        } else if (f == "tan") {
            if (use_native)
                tf._f1 = cftal::native_tan;
            else
                tf._f1 = cftal::tan;
            tf._f1d= std::tan;
        } else if (f == "exp") {
            if (use_native)
                tf._f1= cftal::native_exp;
            else
                tf._f1 = cftal::exp;
            tf._f1d = std::exp;
        } else if (f == "expm1") {
...
template <class _V>
bool cftal::test::read_data(func_data<_V>& tf, std::istream& is)
{
    std::string line;
    while (!getline(is, line).eof()) {
        line = delete_comment(line);
...
template <class _V>
bool cftal::test::test_data(const func_data<_V>& tf, std::ostream& os)
{
    bool rc(true);
    int errs(0);
    std::size_t calls(0);
...
template <class _V>
bool cftal::test::func(std::istream& is, bool use_native)
{
