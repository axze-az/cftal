...
template <class _V>
bool emuvec::test::elements_equal(const _V& v)
{
...
_T emuvec::test::abs_error(const _T& a, const _T& b)
{
        return abs(a-b);
}

template <class _T>
_T emuvec::test::rel_error(const _T& a, const _T& b)
{
        _T ae(a - b);
        _T r((a + b) * _T(0.5));
        _T re(ae / r);
...
int emuvec::test::ulp(double c, double n)
{
...
std::string emuvec::test::delete_comment(const std::string& s)
{
                             ...
                             bool use_native)
{
        ...
        while (!getline(is, line).eof()) {
                ...
                if (f == "cos") {
                        if (use_native)
                                tf._f1 = emuvec::native_cos;
                        else
                                tf._f1 = emuvec::cos;
                        tf._f1d= std::cos;
                } else if (f == "sin") {
                        if (use_native)
                                tf._f1 = emuvec::native_sin;
                        else
                                tf._f1 = emuvec::sin;
                        tf._f1d= std::sin;
                } else if (f == "tan") {
                        if (use_native)
                                tf._f1 = emuvec::native_tan;
                        else
                                tf._f1 = emuvec::tan;
                        tf._f1d= std::tan;
                } else if (f == "exp") {
                        if (use_native)
                                tf._f1= emuvec::native_exp;
                        else
                                tf._f1 = emuvec::exp;
                        tf._f1d = std::exp;
                } else if (f == "expm1") {
...
template <class _V>
bool emuvec::test::read_data(func_data<_V>& tf, std::istream& is)
{
        std::string line;
        while (!getline(is, line).eof()) {
                line = delete_comment(line);
...
template <class _V>
bool emuvec::test::test_data(const func_data<_V>& tf, std::ostream& os)
{
        bool rc(true);
        int errs(0);
        std::size_t calls(0);
...
template <class _V>
bool emuvec::test::func(std::istream& is, bool use_native)
{
