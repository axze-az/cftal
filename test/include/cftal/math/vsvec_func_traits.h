...
//
#if !defined (__CFTAL_MATH_VSVEC_FUNC_TRAITS_H__)
#define __CFTAL_MATH_VSVEC_FUNC_TRAITS_H__ 1

#include <cftal/config.h>
#include <cftal/vsvec.h>
#include <cftal/vsvec_d_real_traits.h>
...
namespace cftal {
    ...
    namespace math {
	// the classes below are required for compile tests to ensure
	// vsvec<_T, _A> supports the same operations as vec<_T, _N>
        struct vsvec_func_traits_f64
            : public func_traits<double, int32_t> {
            using vf_type = vsvec<double>;
            using vmf_type = typename vsvec<double>::mask_type;
            using vi_type = vsvec<int32_t>;
            using vmi_type = typename vi_type::mask_type;
            using vu_type = vsvec<uint32_t>;
            // integer vector with the same length as vf_type
            using vi2_type = vsvec<int32_t>;
            using vmi2_type = typename vi2_type::mask_type;

            using vli_type = vsvec<int64_t>;
            using vmli_type= typename vli_type::mask_type;
            ...
            vmi_to_vmf(const vmi_type& mi) {
                vmi_type t=combine_even_odd(mi, mi);
                return as<vmf_type>(t);
            }

            static
            vmi_type
            vmf_to_vmi(const vmf_type& mf) {
                vmi2_type t=vmf_to_vmi2(mf);
                vmi_type r=even_elements(t);
                return r;
            }
            ...
            vmf_to_vmi2(const vmf_type& mf) {
                return as<vmi2_type>(mf);
            };

            static
            vmf_type
            vmi2_to_vmf(const vmi2_type& mf) {
                return as<vmf_type>(mf);
            };
            ...
            static
            bool any_of_vmi2(const vmi_type& b) {
                return any_of(b);
            }

            static
            bool all_of_vmi2(const vmi_type& b) {
                return all_of(b);
            }

            static
            bool none_of_vmi2(const vmi_type& b) {
                return none_of(b);
            }
            ...
            static
            vi_type sel_vi(const vmi_type& msk,
                           const int32_t& t, const int32_t& f) {
                return select(msk, t, f);
            }
            ...
            static
            vi_type sel_vi2(const vmi_type& msk,
                            const vi_type& t, const vi_type& f) {
                return select(msk, t, f);
            }
            ...
            static
            vi_type sel_val_or_zero_vi2(const vmi_type& msk,
                                        const int32_t& t) {
                return select(msk, t, 0);
            }
            ...
            static
            vi_type sel_zero_or_val_vi2(const vmi_type& msk,
                                        const vi_type& f) {
                return select(msk, 0, f);
            }
            ...
            vf_type insert_exp_vi2(const vi2_type& e) {
                vi2_type ep(e << 20);
                vli_type ri= as<vli_type>(ep) & exp_f64_msk::v.s64();
                vf_type r=as<vf_type>(ri);
                return r;
            }
        ...
        template <>
        struct func_traits<vsvec<double>, vsvec<int32_t> >
            : public vsvec_func_traits_f64 {
        };

        struct vsvec_func_traits_f32 : public func_traits<float, int32_t> {
            using vf_type = vsvec<float>;
            ...
            using vhf_traits = func_traits<vsvec<double>,
                                           vsvec<int32_t> >;
            ...
            vf_type sel_val_or_zero(const vmf_type& msk,
                                    const vf_type& t) {
                return select(msk, t, 0.0f);
            }
            static
            vf_type sel_zero_or_val(const vmf_type& msk,
                                    const vf_type& f) {
                return select(msk, 0.0f, f);
            }
            static
            vf_type insert_exp(const vi_type& e) {
                vi_type ep(e << 23);
        ...
        template <>
        struct func_traits<vsvec<float>, vsvec<int32_t> >
            : public vsvec_func_traits_f32 {
        };
