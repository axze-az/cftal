...
int main(int argc, char** argv)
{
    ...
    auto dp=std::make_pair(-std::numeric_limits<bf16_t>::max(),
    ...
    rc &= of_fp_func_up_to<
        bf16_t, _N, check_sincos<bf16_t> >::v(st, dp,
                                            ags._speed_only,
                                            ags._mt,
                                            cmp,
                                            ags._cnt);
    ...
    auto dp2=std::make_pair(-0x1p18_bf16, 0x1p18_bf16);
    auto us_sin2=std::make_shared<ulp_stats>();
    auto us_cos2=std::make_shared<ulp_stats>();
    exec_stats<_N> st2;
    struct cmp_ulp<std::pair<bf16_t, bf16_t> > cmp2(ulp, us_sin2, us_cos2);
    rc &= of_fp_func_up_to<
        bf16_t, _N, check_sincos<bf16_t> >::v(st, dp2,
                                            ags._speed_only,
                                            ags._mt,
                                            cmp2,
                                            ags._cnt);
