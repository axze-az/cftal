...
#include "cftal/test/check_powi.h"
#include <iostream>
#include <iomanip>
...
int main(int argc, char** argv)
{
    ...
    func_domain<bf16_t> da=std::make_pair(-std::numeric_limits<bf16_t>::max(),
    ...
    rc &= of_fp_func_2_up_to<
        bf16_t, _N, check_powi<bf16_t>, bf16_t, int16_t>::
            v(st, da, db, ags._speed_only, ags._mt,
              cmp_ulp<bf16_t>(ulp3, us), ags._cnt, false);
    std::cout << "ulps: "
    ...
    rc &= of_fp_func_2_up_to<
        bf16_t, _N, check_powi<bf16_t>, bf16_t, int16_t>::
            v(st1, d1, d2, ags._speed_only,
              ags._mt, cmp_ulp<bf16_t>(ulp, us), ags._cnt, true);
