...
namespace cftal {
    ...
    namespace test {
        ...
        struct check_fmaximum_mag {
            template <std::size_t _N>
            static
            vec<_T, _N>
            v(const vec<_T, _N>& a, const vec<_T, _N>& b) {
                return fmaximum_mag(a, b);
            }

            static
            _T
            __fmaximum_mag(const _T& x, const _T& y) {
                using std::signbit;
                using std::fabs;
                using std::isnan;
                if (isnan(x) || isnan(y)) {
            ...
            static
            const char* fname() { return "fmaximum_mag"; }
        };
    }
}
...
int main(int argc, char** argv)
{
    ...
    std::cout << "testing fmaximum_mag vXf64" << std::endl;
    exec_stats<_N64> d_st;
    bool rd= of_fp_func_2_up_to<
        double, _N64, check_fmaximum_mag<double> >::v(d_st, d, d,
                                                      ags._speed_only,
                                                      ags._mt,
                                                      cmp_t<double>(),
                                                      ags._cnt);
    ...
    std::cout << "testing fmaximum_mag vXf32" << std::endl;
    rc &= rd;
    ...
    bool rf= of_fp_func_2_up_to<
        float, _N32, check_fmaximum_mag<float> >::v(f_st, f, f,
                                                    ags._speed_only,
                                                    ags._mt,
                                                    cmp_t<float>(),
                                                    ags._cnt);
    ...
    bool rh= of_fp_func_2_up_to<
        f16_t, _N16, check_fmaximum_mag<f16_t> >::v(h_st, h, h,
                                                    ags._speed_only,
                                                    ags._mt,
                                                    cmp_t<f16_t>(),
                                                    ags._cnt);
