...
{
#if 0
    static const double exp_mx2_c[28]= {
    ...
    vf_type x=min(vf_type(27.5), abs(xc));
    x=_T::sel(isnan(x), vf_type(27.5), x);
    vf_type kf= floor(vf_type(x));
    vf_type xr = x - kf;
    vmf_type xr_gt_half = xr > 0.5;
    xr = _T::sel(xr_gt_half, xr - 1.0, xr);
    kf = _T::sel(xr_gt_half, kf + 1.0, kf);
    ...
    struct alignas(NI*sizeof(int32_t)) v_i {
        int32_t _sc[NI];
    } vk;
    mem<vf_type>::store(v_exp_hi._sc, xc);
    mem<vi_type>::store(vk._sc, k);
    for (std::size_t i=0; i<NI; ++i) {
        int32_t ki=vk._sc[i];
        v_exp_hi._sc[i] = exp_mx2_c[ki];
    }
    vf_type exp_hi(mem<vf_type>::load(v_exp_hi._sc, N));
    vf_type r = exp_hi * base_type::exp_k(-(kf*kf)*xr, false) *
        base_type::exp_k(-xr*xr, false) * 0x1p-64;
    using fc_t = math::func_constants<double>;
    r= _T::sel(vf_type(-(xc*xc)) <= fc_t::exp_lo_zero, vf_type(0), r);
    return r;
#else
