...
int main(int argc, char** argv)
{
    ...
    bool rc= check_func_1<double, 1, check_sin<double> >(v, ulp, 0, false);
    rc &= check_func_1<double, 2, check_sin<double> >(v, ulp, 0, false);
    rc &= check_func_1<double, 4, check_sin<double> >(v, ulp, 0, false);
    rc &= check_func_1<double, 8, check_sin<double> >(v, ulp, 0, false);
    ...
    exec_stats st(_N);
    rc &= of_fp_func_up_to<
        double, _N, check_sin<double> >::v(st, dp, cmp_ulp<double>(ulp, us),
                                           0x80000);
    std::cout << "ulps: "
    ...
    rc &= of_fp_func_up_to<
        double, _N, check_sin<double> >::v(st2, dp2,
