...
root12_k(arg_t<vf_type> xc)
{
    ...
    for (int i=0; i<order5; ++i) {
        ...
        vf_type y3=y1*y2;
        vf_type y4=y2*y2;
        vf_type num= mm*horner(m12,
                               -455.0,
                               -3120.0*y1,
                               858.0*y2,
                               2552.0*y3,
                               165.0*y4);
        vf_type denom= horner(m12,
                              1456.0,
                              24024.0*y1,
                              44616.0*y2,
                              12584.0*y3,
    ...
    for (int i=0; i<halley; ++i) {
        ...
        vf_type m12=m8*m4;
#if 1
        vf_type num= 2.0/11.0 * mm * (mm0 - m12);
        vf_type denom= mm0 + ((13.0/11.0) *m12);
#else
        vf_type num= 2.0 * mm * (mm0 - m12);
        vf_type denom= (11.0 * mm0) + (13.0*m12);
#endif
        mm = mm + num/denom;
    }
    for (int i=0; i<nr; ++i) {
        ...
        vf_type m3=m2*mm;
        vf_type m6=m3*m3;
        vf_type m11=m6*m3*m2;

        mm = mm - 1.0/12.0 * (mm - mm0/m11);
    }
#else
    // faithful: 927090
