               ...
               arg_t<vf_type> kf, arg_t<vi2_type> k2)
{
    ...
    vf_type P= horner(xx,
                      ...
                      expn_c0);
    vf_type br= xrh- xx*P;
    vf_type ym1 = xrh + (xrh*br)/(2.0-br);
    vf_type ye = xrl + xrl *ym1;
    vf_type y=ym1;
    if (_EXP_M1 == false) {
        y += ye;
        y += 1.0;
        ...
        // d_ops::add12(ym1, ye, ym1, ye);
        d_ops::add122cond(y, ye, vf_type(1), ym1, ye);
        // 2^kf = 2*2^s ; s = kf/2
        ...
        y = _T::sel((abs(xrh) < 0x1p-2) & (kf==0.0), ym1, y);
    }
    return y;
}
