...
exp_k(arg_t<vf_type> xc, bool exp_m1)
{
...
#if 1
    // [-0.3465735912322998046875, 0.3465735912322998046875] : | p - f | <= 2^-67.5625
    ...
    // x^5 : +0x8.888888888967p-10
    const vf_type exp_c5=+8.3333333333341051696319e-03;
    // x^6 : +0xb.60b60b60a8058p-13
    const vf_type exp_c6=+1.3888888888873320275746e-03;
    // x^7 : +0xd.00d00cfa36d1p-16
    const vf_type exp_c7=+1.9841269838925557631240e-04;
    // x^8 : +0xd.00d00d6bb68a8p-19
    const vf_type exp_c8=+2.4801587349060594630450e-05;
    // x^9 : +0xb.8ef1ea0a352a8p-22
    const vf_type exp_c9=+2.7557322545117379635502e-06;
    // x^10 : +0x9.3f2661413d978p-25
    const vf_type exp_c10=+2.7557251992299936602481e-07;
    // x^11 : +0xd.72d403fbf1978p-29
    const vf_type exp_c11=+2.5049871952748675537375e-08;
    // x^12 : +0x8.fc5d919d0e45p-32
    const vf_type exp_c12=+2.0921702372431042716761e-09;
    // x^13 : +0xb.709edc9d78f08p-36
    const vf_type exp_c13=+1.6647280482514369114429e-10;
#if 1
    vf_type xx=xr*xr;
...
#if 0
    // [0, 0.5] : | p - f | <= 2^-54.4375
    // coefficients for exp2n generated by sollya
    // x^0 : +0xb.8aa3b295c17fp-2
    const vf_type exp2n_c0=+2.8853900817779267740093e+00;
    // x^2 : +0xe.c981ff517c5ap-7
    const vf_type exp2n_c2=+1.1552453009332702693612e-01;
    // x^4 : -0xf.2804b6f4c0208p-14
    const vf_type exp2n_c4=-9.2506847779461759146563e-04;
    // x^6 : +0xb.18a281409d64p-20
    const vf_type exp2n_c6=+1.0582189205047233732972e-05;
    // x^8 : -0x8.87a5d0aa2ee7p-26
    const vf_type exp2n_c8=-1.2710502576210040801092e-07;
    // x^10 : +0xd.2984303a5d2c8p-33
    const vf_type exp2n_c10=+1.5322786388881641502606e-09;
    vf_type xx= xr*xr;
    vf_type P= impl::poly(xx,
                          exp2n_c10,
                          exp2n_c8,
                          ...
                          exp2n_c0);
    // vf_type y = (xr + P)/(P-xr);
    // vf_type y= 1 + 2*xr/(P-xr);
    vf_type y = 1.0+xr+((xr + 2-P)*xr)/(P - xr);
    y = scale_exp_k(y, kf, k2);
    return y;
#else
...
log2_k(arg_t<vf_type> xc)
{
...
#if 1
    dvf_type df = vf_type(xr-1);
    dvf_type ds = df/(vf_type(2)+df);
    vf_type s = ds.h();
    vf_type z = sqr(ds).h();
#else
    vf_type f = 1-xr;
    ...
    vf_type z = s*s;
#endif
    // [-0.17157287895679473876953125, 3.4694469519536141888238489627838134765625e-18] : | p - f | <= 2^-68
    // coefficients for log2 generated by sollya
    // x^1 h: +0xb.8aa3b295c17fp-2
    const vf_type log2_c1h=+2.8853900817779267740093e+00;
    // x^1 l: +0xb.be91c4fed94ap-58
    const vf_type log2_c1l=+4.0746613604942545693527e-17;
    // x^3 : +0xf.6384ee1d01fep-4
    const vf_type log2_c3=+9.6179669392597544330670e-01;
    // x^5 : +0x9.3bb62877d052p-4
    const vf_type log2_c5=+5.7707801635571742693287e-01;
    // x^7 : +0xd.30bb15377e438p-5
    const vf_type log2_c7=+4.1219858306789841639883e-01;
    // x^9 : +0xa.4258a29f5ee68p-5
    const vf_type log2_c9=+3.2059890520301054239738e-01;
    // x^11 : +0x8.64d2b4ad5f59p-5
    const vf_type log2_c11=+2.6230750359080012845681e-01;
    // x^13 : +0xe.351bbe88f0be8p-6
    const vf_type log2_c13=+2.2199147803236121689885e-01;
    // x^15 : +0xc.3b155b21a764p-6
    const vf_type log2_c15=+1.9110616588169748730763e-01;
    // x^17 : +0xc.432e1dfaa69d8p-6
    const vf_type log2_c17=+1.9160035064360578238585e-01;
#if 0
    vf_type p= impl::poly(z,
                          log2_c17,
                          log2_c15,
                          log2_c13,
                          log2_c11,
    ...
    vf_type p= impl::poly(z,
                          ...
                          log2_c3)*z;
    vf_type th, tl;
    d_ops::add212(th, tl, log2_c1h, log2_c1l, p);
    vf_type ph, pl;
    d_ops::mul122(ph, pl, s, th, tl);
    vf_type l2= pl + ph + kf;
#endif
    return l2;
#else
