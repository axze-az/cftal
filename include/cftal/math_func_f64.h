    ...
    namespace math {
        ...
        struct func_core<double, _T> {
            ...
            log_k(arg_t<vf_type> x, log_func f=log_func::c_log_e,
                  vf_type* hi=nullptr, vf_type* lo=nullptr);
...
log2_k(arg_t<vf_type> xc)
{
#if 0
    using fc=func_constants<double>;   
    vmf_type is_denom= abs(xc) <= fc::max_denormal;
    // denormal handling
    vf_type xs= _T::sel(is_denom, xc*vf_type(0x1.p54), xc);
    vmi2_type i_is_denom= _T::vmf_to_vmi2(is_denom);
    vi2_type eo= _T::sel(i_is_denom, vi2_type(-54), vi2_type(0));
    // extract mantissa
    vi2_type lo_word, hi_word;
    _T::extract_words(lo_word, hi_word, xs);
    // exponent:
    vi2_type e=((hi_word >> 20) & _T::e_mask) + eo - _T::bias;
    // insert exponent of 1.0
    hi_word = (hi_word & vi2_type(0x800fffff)) | vi2_type(0x3ff00000);
    // combine low and high word, xr [1, 2)
    vf_type xr(_T::combine_words(lo_word, hi_word));
        
    // [0, 0.25] : | p - f | <= 2^-61.34375
    // coefficients for log2 generated by sollya
    // x^1 : +0xb.8aa3b295c17fp-3
    const vf_type log2_c1=+1.4426950408889633870047e+00;
    // x^3 : +0xf.6384ee1d03b58p-7
    const vf_type log2_c3=+1.2022458674075912898882e-01;
    // x^5 : +0x9.3bb628732b0cp-9
    const vf_type log2_c5=+1.8033688009003595587032e-02;
    // x^7 : +0xd.30bb204e71848p-12
    const vf_type log2_c7=+3.2203015915935923225988e-03;
    // x^9 : +0xa.4251d577ea688p-14
    const vf_type log2_c9=+6.2616340244283686961718e-04;
    // x^11 : +0x8.670fd7b5161b8p-16
    const vf_type log2_c11=+1.2821327955622890157165e-04;
    // x^13 : +0xd.73e1d755ff398p-19
    const vf_type log2_c13=+2.5658921837921942423920e-05;
    // x^15 : +0xc.c60bdcdeb7abp-20
    const vf_type log2_c15=+1.2181871902043329259453e-05;
    // x^ h: +0x0p+0
    const vf_type log2_1_00h=+0.0000000000000000000000e+00;
    // x^ l: +0x0p+0
    const vf_type log2_1_00l=+0.0000000000000000000000e+00;
    // x^ h: +0xa.4d3c25e68dc58p-5
    const vf_type log2_1_25h=+3.2192809488736234824202e-01;
    // x^ l: -0xd.b6a048059281p-65
    const vf_type log2_1_25l=-3.7170199641426819258679e-19;
    // x^ h: +0x9.5c01a39fbd688p-4
    const vf_type log2_1_50h=+5.8496250072115618667823e-01;
    // x^ l: -0xc.0bfe9dbebf2e8p-61
    const vf_type log2_1_50l=-5.2244900613901090572402e-18;
    // x^ h: +0xc.eaecfea808598p-4
    const vf_type log2_1_75h=+8.0735492205760406303483e-01;
    // x^ l: +0xc.ccab240e904f8p-58
    const vf_type log2_1_75l=+4.4407139084295173616539e-17;
    // x^ : +0xap-3
    const vf_type log2_i1_l=+1.2500000000000000000000e+00;
    // x^ : +0xcp-3
    const vf_type log2_i2_l=+1.5000000000000000000000e+00;
    // x^ : +0xep-3
    const vf_type log2_i3_l=+1.7500000000000000000000e+00;

    vf_type kf= _T::cvt_i_to_f(_T::vi2_odd_to_vi(e));    
    vmf_type si= xr < log2_i1_l;
    vf_type tkh= _T::sel(si, log2_1_00h, log2_1_25h);
    vf_type tkl= _T::sel(si, log2_1_00l, log2_1_25l);
    vf_type ck= _T::sel(si, 1.00, 1.25);

    si = xr < log2_i2_l;
    tkh = _T::sel(si, tkh, log2_1_50h);
    tkl = _T::sel(si, tkl, log2_1_50l);
    ck = _T::sel(si, ck, 1.50);

    si = xr < log2_i3_l;
    tkh = _T::sel(si, tkh, log2_1_75h);
    tkl = _T::sel(si, tkl, log2_1_75l);
    ck = _T::sel(si, ck, 1.75);

    vf_type r= 2.0 * (xr -ck)/(xr +ck);
    vf_type r2=r*r;
#if 1
    vf_type p= impl::poly(r2,
                          log2_c15,
                          log2_c13,
                          log2_c11,
                          log2_c9,
                          ...
                          log2_c3,
                          log2_c1)*r;
    vf_type l2= ((p+tkl)+tkh) + kf;
#else
    vf_type ph, pl;
    impl::eft_poly(ph, pl, r2,
                   log2_c15,
                   log2_c13,
                   log2_c11,
                   log2_c9,
                   ...
                   log2_c1);
    vf_type pe;
    ph = d_ops::two_prod(ph, r, pe);
    pl = pl * r + pe;
    vf_type l2= ((((pl+tkl)+ph))+tkh) + kf;
#endif
    return l2;
#else    
/*
...
pow_k(arg_t<vf_type> x, arg_t<vf_type> y)
{
    ...
    vf_type xa= abs(x);
    vf_type logxh, logxl;
    log_k(xa, log_func::c_log_2, &logxh, &logxl);
    vf_type th, tl;
    th = d_ops::two_sum(logxh, logxl, tl);
    dvf_type lnxy(th, tl);
    lnxy *= y;
    vi2_type lu, hu;
    _T::extract_words(lu, hu, lnxy.h());
    vf_type ah=_T::combine_words(0, hu);
    vf_type al=lnxy.h() - ah + lnxy.l();
    vf_type eh=exp2_k(ah);
    vf_type el=exp2_k(al);
    vf_type t;
    vf_type pk=eh*el;

    using fc=func_constants<double>;
    const vf_type& d= lnxy.h();
    const vf_type exp2_hi_inf= fc::exp_hi_inf;
    const vf_type exp2_lo_zero= fc::exp_lo_zero;
    pk = _T::sel(d <= exp2_lo_zero, 0.0, pk);
    pk = _T::sel(d >= exp2_hi_inf, _T::pinf(), pk);
    pk = _T::sel(d == 0.0, 1.0, pk);
    pk = _T::sel(d == 1.0, 2.0, pk);
    return pk;
#else
    /*
     * z = x^y
     * log2(z) = log2(x^y) = y*log2(x);
     * 2^log2(z) = z = 2^(y*log2(x))
     * z= 2^(y*log2(x))
     * x= 2^i*f
     * y= 2^j*g
     * z= 2^(2^j*g*log2(2^i*f)
     *  = 2^(2^j*g*(i+log2(f))
     */
    vf_type xa= abs(x);
    vf_type xe= rint(xa);
