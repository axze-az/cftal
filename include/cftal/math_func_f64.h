...
exp10_k(arg_t<vf_type> x)
{
#if 0
    using ctbl = impl::d_real_constants<d_real<double>, double>;
    vf_type xl, xh=d_ops::two_prod(x, ctbl::m_ln10.h(), xl);
    vf_type kf = rint(vf_type(xh * ctbl::m_1_ln2.h()));
    vf_type hi = xh - kf * ctbl::m_ln2_cw[0];
    vf_type xr = hi - kf * ctbl::m_ln2_cw[1];
    vi_type k= _T::cvt_f_to_i(kf);
    vi2_type k2= _T::vi_to_vi2(k);
    vf_type dx= hi - xr;
    ...
    // xl += t;
    // vf_type tl, th=d_ops::two_prod(xr, ctbl::m_ln10.h(), tl);
    // tl += dx;
    // vf_type xl, xh=d_ops::two_sum(th, tl, xl);
