...
sinh_k(arg_t<vf_type> xc)
{
    ...
    // (e^v/2-1) = 5.850000830633739e-2
    // (e-v)/2 = 2.361832763705074e-1
    ...
    // vf_type t= exp_k(xx, true);
    // vf_type t1= t+1;
    // here we produce most of the 2 ulp deviations:
    // vf_type sinh_le_1 = h * (2*t - t*t/(t1));
    ...
    vf_type xr= _T::sel(x_large, x - 0.75, x);
    vf_type em=exp_k(xr, true);
    vf_type e= em+1;
    const vf_type e_v_2_m_1 = 5.850000830633739e-2;
    const vf_type e_m_v_2 = 2.361832763705074e-1;
    vf_type sinh_gt_1 = 0.5 * (em + em/e);
    vf_type sinh_huge = e + e_v_2_m_1 * e - e_m_v_2 / e;
    // vf_type sinh_gt_1 = h * (t + t/(t1));
    // vf_type sinh_l_h= (t1 - 1/t1)*0.5; // --> t1*0.5
    // vf_type cosh_l_h= (t1 + 1/t1)*0.5; // --> t1*0.5
    // vf_type sinh_l= t1 * (t1*0.5);
    vf_type sinh_x = _T::sel(absx<=1.0, sinh_le_1, sinh_gt_1);
    sinh_x = _T::sel(x_large, sinh_huge, sinh_x);
    vf_type y = copysign(sinh_x, xc);
#else
    vf_type x=abs(xc);
    vf_type h=copysign(vf_type(0.5), xc);
    vf_type t=exp_k(x, true);
    vf_type yl= 2*h*__expo_k(x);
    vf_type ys= _T::sel(x< 1.0,
                        h*(2*t - t*t/(t+1)),
                        h*(t+t/(t+1)));
    vf_type y= _T::sel(x < 0x1.62e42p+9, ys, yl);
#endif
    return y;
}
