...
namespace cftal {
    ...
    namespace math {
        ...
        struct elem_func : public elem_func_wrapper< _FLOAT_T, _TRAITS_T> {
            ...
            // base 2 logarithmus of 1+x function with error handling
            static
            vf_type
            log2p1(arg_t<vf_type> vf);
	    ...
	    // arc hyperbolic tangent function with error handling
            ...
            // returns e^(-x*x) with error handling
            static
            vf_type
            ...
            // returns 2^(-x*x) with error handling
            static
            vf_type
            ...
            // returns 10^(-x*x) with error handling
            static
            vf_type
...
exp(arg_t<vf_type> x)
{
    __asm__ volatile("# LLVM-MCA-BEGIN exp\n\t");
#if 0
...
#endif
    __asm__ volatile("# LLVM-MCA-END exp\n\t");
    return y;
}
...
{
    __asm__ volatile("# LLVM-MCA-BEGIN expm1\n\t");
    ...
    __asm__ volatile("# LLVM-MCA-END expm1\n\t");
    return y;
}
