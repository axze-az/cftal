...
rsqrt_k(arg_t<vf_type> x)
{
    ...
    // x^0 : +0xf.615f3p-2f
    const float rsqrt_c0=+3.8450896740e+00f;
    // x^1 : -0xc.727b7p+0f
    const float rsqrt_c1=-1.2447196007e+01f;
    // x^2 : +0xd.ce85bp+1f
    const float rsqrt_c2=+2.7613454819e+01f;
    // x^3 : -0x8.b3e59p+2f
    const float rsqrt_c3=-3.4810886383e+01f;
    // x^4 : +0xb.730f9p+1f
    const float rsqrt_c4=+2.2898912430e+01f;
    // x^5 : -0xc.33363p-1f
    const float rsqrt_c5=-6.1000227928e+00f;
...
exp10_k(arg_t<vf_type> x, bool exp_m1)
{
    ...
    // x^ : +0x9.35d8ep-2f
    const float log10=+2.3025851250e+00f;
    // x^ : +0xa.9a926p-1f
    const float log10sqr=+5.3018980026e+00f;
#if 0
    vf_type y=impl::poly(xr,
                         exp10_c6,
                         exp10_c5,
    ...
    if (exp_m1 == false) {
        ...
        y  = y + 2*ye;
        // x small
        y = _T::sel(abs(x) < 0x1p-25f/log10, log10*x, y);
    }
#endif
    return y;
#endif
...
cbrt_k(arg_t<vf_type> xc)
{
    ...
    vi_type e3 = (((e+v_bias_3)*fac_1_3)>>shift_1_3) - v_bias;
    vi_type r = e - e3 - (e3<<1);
...
#endif
    mm0 = mm0 * scale;
    vi_type e3c=_T::sel(r != 0, e3+1, e3);
#else
    // m in [0.5, 1)
