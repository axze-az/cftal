...
void
cftal::math::elem_func_core<float, _T>::
__reduce_log_arg(vhf_type& xr, vhf_type& inv_c, vhf_type& log_c,
                 vi_type& ki,
                 arg_t<vf_type> xc)

{
    ...
    hx = m + offs.s32();
    xr = cvt<vhf_type>(_T::as_float(hx));
    auto lck=make_variable_lookup_table<double>(idx);
    const auto& tbl=log_data<double>::_tbl;
    inv_c =lck.from(tbl._inv_c);
    switch (_LFUNC) {
    case log_func::c_log_e:
        log_c=lck.from(tbl._log_c_h);
        break;
    case log_func::c_log_2:
        log_c=lck.from(tbl._log2_c_h);
        break;
    case log_func::c_log_10:
        log_c=lck.from(tbl._log10_c_h);
        break;
    }
    ki = k;
}
...
__log_tbl_k_d(arg_t<vf_type> xc)
{
    vhf_type xr, inv_c, log_c;
    vi_type ki;
    __reduce_log_arg<_LFUNC>(xr, inv_c, log_c, ki, xc);
    vhf_type r;
    r = xr * inv_c - 1.0;
    // [-1.5625e-2, 1.5625e-2] : | p - f | <= 2^-34.84375
    // coefficients for log generated by sollya
    // x^1 : +0x8p-3
    constexpr
    const double log_c1=+1.0000000000000000000000e+00;
    // x^2 : -0xf.ffffff1db9fbp-5
    constexpr
    const double log_c2=-4.9999999835364350442291e-01;
    // x^3 : +0xa.aaff82ae13568p-5
    constexpr
    const double log_c3=+3.3337379001077743767567e-01;
    // x^4 : -0x8.005ab423a7878p-5
    constexpr
    const double log_c4=-2.5004325087978512831555e-01;
    ...
    // vhf_type p=horner2(r, r2, ci);
    vhf_type p=horner(r, ci);
    vhf_type ll=p*r2+r;
    vhf_type lh;
    if (_LFUNC==log_func::c_log_e) {
        lh = (kf * ctbl::m_ln2[0] + log_c) + ll;
    } else if (_LFUNC==log_func::c_log_2) {
        lh = kf + log_c + ll * ctbl::m_1_ln2[0];
    } else if (_LFUNC==log_func::c_log_10) {
        lh = kf * ctbl::m_lg2[0] + log_c + ll * ctbl::m_1_ln10[0];
    }
...
__log_tbl_k12_d(arg_t<vf_type> xc)
{
    ...
    r = xr * inv_c - 1.0;
    vhf_type kf=cvt<vhf_type>(ki);
    // [-1.5625e-2, 1.5625e-2] : | p - f | <= 2^-42.5625
    // coefficients for log generated by sollya
    // x^1 : +0x8p-3
    constexpr
    const double log_hp_c1=+1.0000000000000000000000e+00;
    // x^2 : -0xf.fffffe560643p-5
    constexpr
    const double log_hp_c2=-4.9999999690062002510871e-01;
    // x^3 : +0xa.aaaaa896905dp-5
    constexpr
    const double log_hp_c3=+3.3333332946177629541040e-01;
    // x^4 : -0x8.006aa0c3a52c8p-5
    constexpr
    const double log_hp_c4=-2.5005084418549966551737e-01;
    // x^5 : +0xc.cd9c481bedfcp-6
    constexpr
    const double log_hp_c5=+2.0004946748642260523354e-01;
    static_assert(log_hp_c1 == 1.0, "constraint violated");
    using ctbl=impl::d_real_constants<d_real<double>, double>;
    static const double ci[]={
    ...
    vhf_type p=horner2(r, r2, ci);
    if (_LFUNC==log_func::c_log_e) {
        lh = (kf * ctbl::m_ln2[0] + log_c) + (p*r2 +r);
    } else if (_LFUNC==log_func::c_log_2) {
        lh = kf + log_c + (p*r2 +r) * ctbl::m_1_ln2[0];
    } else if (_LFUNC==log_func::c_log_10) {
        lh = kf * ctbl::m_ln10[0] + log_c + (p*r2 +r) * ctbl::m_1_ln10[0];
    }
    return lh;
}
#endif
...
atan2_k(arg_t<vf_type> y, arg_t<vf_type> x)
{
    ...
    if (_T::any_of_v(s= (y_s >=0.0f) & x_lt_0)) {
...
acosh_k(arg_t<vf_type> xc)
{
    ...
    if (_T::any_of_v(sel = x > acosh_i1_right)) {
        using ctbl=impl::d_real_constants<d_real<float>, float>;
        vmf_type x_huge = x > 0x1p24f;
        vf_type add_2_log=_T::sel_val_or_zero(x_huge, ctbl::m_ln2[0]);
        // vf_type t= x*x;
