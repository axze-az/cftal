...
sinh_cosh_k(arg_t<vf_type> xc)
{
    ...
    if (_F == hyperbolic_func::c_cosh) {
        ...
        if (__unlikely(_T::any_of_vmf(x_small))) {
            ...
            vf_type p=horner2(xs2, vf_type(xs2*xs2), cs)*xs2;
            vf_type xs2h=cosh_c2 * xs2;
            vf_type xs2lh=cosh_c2 * xs2l;
            vf_type yt, e1;
            d_ops::add12(yt, e1, cosh_c0, xs2h);
            vf_type e2;
            d_ops::add12(yt, e2, yt, xs2lh);
            yt += (e1+e2) + xs2*p;
#else
            vf_type xs2=x*x;
            vf_type p=horner2(xs2, vf_type(xs2*xs2), cs)*xs2;
            vf_type yt, e;
            d_ops::add12(yt, e, cosh_c0, xs2*cosh_c2);
            yt += (e + xs2*p);
            // vf_type yt= cosh_c0 + ((xs2*cosh_c2)
#endif
            res = yt;
        }
    }
