...
#else
    // [-2.70760618150234222412109375e-3, 2.70760618150234222412109375e-3] : | p - f | <= 2^-64.65625
    // coefficients for exp generated by sollya
    // x^1 : +0x8p-3
    constexpr
    const double exp_c1=+1.0000000000000000000000e+00;
    // x^2 : +0xf.ffffffffff2ep-5
    constexpr
    const double exp_c2=+4.9999999999997668531648e-01;
    // x^3 : +0xa.aaaaaaaaaae7p-6
    constexpr
    const double exp_c3=+1.6666666666667001583946e-01;
    // x^4 : +0xa.aaaae152a73d8p-8
    constexpr
    const double exp_c4=+4.1666679392304360740606e-02;
    // x^5 : +0x8.888894fb47898p-10
    constexpr
    const double exp_c5=+8.3333340579155087274321e-03;
#endif    
...
#else
    vf_type p4=horner(xrh, exp_c5, exp_c4);
#endif    
...
#else
    static_assert(exp_data<double>::EXP_N==128,
                 "exp_data<double>::EXP_N==128");
    // x^ : +0xb.8aa3b295c17fp+4
    constexpr
    const double _128_ln2=+1.8466496523378731353660e+02;
    // x^ : +0xb.17217f7ep-11
    constexpr
    const double _ln2_128_cw_h=+5.4152123482253955444321e-03;
    // x^ : -0xe.308654361c4c8p-47
    constexpr
    const double _ln2_128_cw_l=-1.0082281460997768735671e-13;    
    vf_type kf = rint(vf_type(x * _128_ln2));
    vi_type ki=_T::cvt_f_to_i(kf);
    idx = ki & exp_data<double>::EXP_IDX_MASK;
    k = ki >> exp_data<double>::EXP_SHIFT;
    vf_type hi = x - kf * _ln2_128_cw_h;
    xrh = hi - kf * _ln2_128_cw_l;
    vf_type dx = hi-xrh;
    xrl = dx - kf * _ln2_128_cw_l;
#endif    
}
                 ...
                 arg_t<vf_type> xl)
{
    ...
    constexpr
    const double _128_ln2=+1.8466496523378731353660e+02;
    // x^ : +0xb.17217f7d1cf78p-11
    constexpr
    const double _ln2_128_h=+5.4152123481245725486466e-03;
    // x^ : +0xd.5e4f1d9cc01f8p-66
    constexpr
    const double _ln2_128_l=+1.8117553233174215300139e-19;    
    vf_type kf = rint(vf_type(xh * _128_ln2));
    vi_type ki=_T::cvt_f_to_i(kf);
    idx = ki & exp_data<double>::EXP_IDX_MASK;
    k = ki >> exp_data<double>::EXP_SHIFT;
    vf_type neg_kfln2h, neg_kfln2l;
    d_ops::unorm_mul122(neg_kfln2h, neg_kfln2l,
                        kf, -_ln2_128_h, -_ln2_128_l);
    d_ops::add22cond(xrh, xrl,
                     xh, xl,
                     neg_kfln2h, neg_kfln2l);
#endif    
...
#else
    static_assert(exp_data<double>::EXP_N == 128,
                  "exp_data<double>::EXP_N == 128 expected");
#endif    
                   ...
                   arg_t<vf_type> x)
{
    ...
    static_assert(exp_data<double>::EXP_N == 128,
                  "exp_data<double>::EXP_N == 128 expected");
    constexpr
    const double _128_lg2=+4.2520679614558235925870e+02;
    // x^ : +0x9.a209a85p-12
    constexpr
    const double _lg2_128_cw_h=+2.3517968411397305317223e-03;
    // x^ : -0x8.6010cee0ed4c8p-49
    constexpr
    const double _lg2_128_cw_l=-1.4877444114972089181230e-14;    
    vf_type kf= rint(vf_type(x*_128_lg2));
    vi_type ki=_T::cvt_f_to_i(kf);
    idx = ki & exp_data<double>::EXP_IDX_MASK;
    k = ki >> exp_data<double>::EXP_SHIFT;
    vf_type hi = x - kf * _lg2_128_cw_h;
    xrh = hi - kf * _lg2_128_cw_l;
    vf_type dx= hi-xrh;
    vf_type cr = dx- kf * _lg2_128_cw_l;    
#endif    
    using ctbl = impl::d_real_constants<d_real<double>, double>;
                   ...
                   arg_t<vf_type> xl)
{
    ...
    static_assert(exp_data<double>::EXP_N==128,
                 "exp_data<double>::EXP_N==128");   
    using ctbl = impl::d_real_constants<d_real<double>, double>;
    // x^ : +0xd.49a784bcd1b88p+5
    constexpr
    const double _128_lg2=+4.2520679614558235925870e+02;
    // x^ : +0x9.a209a84fbcff8p-12
    constexpr
    const double _lg2_128_h=+2.3517968411248531095115e-03;
    // x^ : -0xc.ee0ed4ca7e908p-69
    constexpr
    const double _lg2_128_l=-2.1904125998321643701036e-20;    
    vf_type kf = rint(vf_type(xh*_128_lg2));
    vi_type ki=_T::cvt_f_to_i(kf);
    idx = ki & exp_data<double>::EXP_IDX_MASK;
    k = ki >> exp_data<double>::EXP_SHIFT;
    vf_type kf_lg_2_128_h, kf_lg_2_128_l;
    d_ops::unorm_mul122(kf_lg_2_128_h, kf_lg_2_128_l,
                        kf, -_lg2_128_h, -_lg2_128_l);
    d_ops::add22cond(xrh, xrl,
                     xh, xl,
                     kf_lg_2_128_h, kf_lg_2_128_l);
#endif    
...
sinh_cosh_k(arg_t<vf_type> xc)
{
    ...
    if (_F == hyperbolic_func::c_sinh) {
        ...
        if (__unlikely(_T::any_of_v(x_tiny))) {
    ...
    if (_F == hyperbolic_func::c_cosh || __likely(!_T::all_of_v(x_tiny))) {
        vf_type xrh, xrl;
        vi_type k, idx;
