...
rsqrt_k(arg_t<vf_type> x)
{
    ...
    // x^0 : +0xe.bce501ede0ff8p-2
    const double rsqrt_i0_c0=+3.6844673444213635882249e+00;
    // x^1 : -0xa.4b6317d48265p+0
    const double rsqrt_i0_c1=-1.0294480790502706923917e+01;
    // x^2 : +0x8.756031be8275p+1
    const double rsqrt_i0_c2=+1.6916998117468132534214e+01;
    // x^3 : -0xa.d2d261a856bb8p+0
    const double rsqrt_i0_c3=-1.0823522666548099024908e+01;
    // [0.5, 1] : | p - f | <= 2^-10.52734375
    // coefficients for rsqrt generated by sollya
    // x^0 : +0xa.6bd6de3c9cc9p-2
    const double rsqrt_i1_c0=+2.6053118443007372562192e+00;
    // x^1 : -0xe.8f000a17804bp-2
    const double rsqrt_i1_c1=-3.6396485878795585833245e+00;
    // x^2 : +0xb.f64dc39cd83bp-2
    const double rsqrt_i1_c2=+2.9905310215454461442164e+00;
    // x^3 : -0xf.4e88a54b9e118p-4
    const double rsqrt_i1_c3=-9.5667328423030928430393e-01;
    ...
    vf_type mm_i1= horner(mm0,
                          ...
                          rsqrt_i1_c1,
                          rsqrt_i1_c0);
    vf_type mm = _T::sel(mm0 <= 0.5, mm_i0, mm_i1);
#else
    // [0.25, 1] : | p - f | <= 2^-7.8828125
    // coefficients for rsqrt generated by sollya
    // x^0 : +0xd.fdfba62a869bp-2
    const vf_type rsqrt_c0=+3.4980302775817575522410e+00;
    // x^1 : -0x8.fc0dd788e5af8p+0
    const vf_type rsqrt_c1=-8.9845862111303045338673e+00;
    // x^2 : +0xe.a85d5624cb3cp+0
    const vf_type rsqrt_c2=+1.4657674201949006942414e+01;
    // x^3 : -0xb.f932c1c4b3f88p+0
    const vf_type rsqrt_c3=-1.1973430738948055207516e+01;
    // x^4 : +0xf.37bc8c2b592bp-2
    const vf_type rsqrt_c4=+3.8044301892717085067375e+00;
...
template <typename _T>
template <enum cftal::math::elem_func_core<double, _T>::log_func _F>
inline
__attribute__((__always_inline__))
typename cftal::math::elem_func_core<double, _T>::vf_type
cftal::math::elem_func_core<double, _T>::
nlog_k(arg_t<vf_type> xc)
{
    ...
    // x^1 : +0x8p-2
    const vf_type log_c1=+2.0000000000000000000000e+00;
    // x^3 : +0xa.aaaaaaaaaaaa8p-4
    const vf_type log_c3=+6.6666666666666662965923e-01;
    // x^5 : +0xc.cccccccccde78p-5
    const vf_type log_c5=+4.0000000000003138600491e-01;
    // x^7 : +0x9.2492492337668p-5
    const vf_type log_c7=+2.8571428570442664041096e-01;
    // x^9 : +0xe.38e38fe69c078p-6
    const vf_type log_c9=+2.2222222378553666000123e-01;
    // x^11 : +0xb.a2e82369f50d8p-6
    const vf_type log_c11=+1.8181804140461185759214e-01;
    // x^13 : +0x9.d8bcabeda57cp-6
    const vf_type log_c13=+1.5385357656358400468832e-01;
    // x^15 : +0x8.84d39be12c3ap-6
    const vf_type log_c15=+1.3310709211936322393655e-01;
    // x^17 : +0xf.83603792c34dp-7
    const vf_type log_c17=+1.2119677270450282535741e-01;
