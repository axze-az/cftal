...
{
#if 1
   // [0, 0.785398185253143310546875] : | p - f | <= 2^-65.28125
    // coefficients for sin generated by sollya
    // x^1 : +0x8p-3
    const vf_type sin_c1=+1.0000000000000000000000e+00;
    // x^3 : -0xa.aaaaaaaaaaaa8p-6
    const vf_type sin_c3=-1.6666666666666665741481e-01;
    // x^5 : +0x8.888888888845p-10
    const vf_type sin_c5=+8.3333333333330990300158e-03;
    // x^7 : -0xd.00d00d0034f1p-16
    const vf_type sin_c7=-1.9841269841054583753306e-04;
    // x^9 : +0xb.8ef1d1fe4dap-22
    const vf_type sin_c9=+2.7557319127903946807262e-06;
    // x^11 : -0xd.7321e6e7c3228p-29
    const vf_type sin_c11=-2.5052085617685156984916e-08;
    // x^13 : +0xb.08a33376b0618p-36
    const vf_type sin_c13=+1.6056205277110718384464e-10;
    // x^15 : -0xd.29f3ae84d82c8p-44
    const vf_type sin_c15=-7.4827963418787432667734e-13;
#if 0
    static_cast<void>(sin_c1);
    // s = s1*x+s3*x^3+s5*x^5 + ... + cos(x)*xl
    //   = s1*x+s3*x^3+s5*x^5 + ... + (1-x*x/2)*xl
    //   = s1*x+s3*x^3+s5*x^5 + ... + (1-x*x/2)*xl
    //   = s1*x+s3*x^3+s5*x^5 + ... + xl - x^2*xl/2
    //   = s1*x+s3^x^3+x^2*(s5*x^3+...) + xl - x^2*xl/2
...
#else
    // sin(x+y)/(x+y) = sin(x)/x + (cos(x)*x - sin(x))*y/x^2
    //                = sin(x)/x + ((1-x^2/2)*x - x + x^3/6)*y/x^2
    //                = sin(x)/x + (x - x^2/2 - x + x^3/6)*y/x^2
    //                = sin(x)/x + (1/6*x^3-1/2*x^2)*y/x^2
    //                = sin(x)/x + (1/6*x*y) - 1/2*y
    //                = (1+s3*x^2+s5*x^4 + s7*x^6+ ...) + 1/6*x*y - 1/2y
    //                = 1 + x * (s3*x + s5*x^3 + ...+ 1/6*y) - 1/2y
    //                = 1 + x * (P + 1/6*y) - 1/2 y
    vf_type x2h, x2l;
    d_ops::sqr22(x2h, x2l, xh, xl);
    vf_type s= horner(x2h,
                      sin_c15,
                      sin_c13,
                      sin_c11,
                      sin_c9,
                      sin_c7)*x2h;
    vf_type sh, sl;
    d_ops::add12(sh, sl, sin_c5, s);
    d_ops::mul22(sh, sl, x2h, x2l, sh, sl);
    d_ops::add122(sh, sl, sin_c3, sh, sl);
    d_ops::mul22(sh, sl, x2h, x2l, sh, sl);
    d_ops::add122(sh, sl, sin_c1, sh, sl);
    d_ops::mul22(sh, sl, xh, xl, sh, sl);
    s=sh;
#endif
    return s;
#else
