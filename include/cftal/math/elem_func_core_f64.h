...
sinh_k(arg_t<vf_type> xc)
{
    ...
    vf_type yh, yl;
    d_ops::add12(yh, yl, th, tl + th*eh);
    yh *= sc.f0();
    yh *= sc.f1();

    const int NN=16;

    vmf_type k_gt_ln2h_and_lt_36=(x > M_LN2/NN) & _T::vmi_to_vmf(k <36);
    ...
    vmf_type x_lt_ln_2= x <= M_LN2/NN;
    if (any_of(x_lt_ln_2)) {
        // [0, 4.33216989040374755859375e-2] : | p - f | <= 2^-66
        // coefficients for sinh generated by sollya
        // x^1 : +0x8p-3
        // const double sinh_c1=+1.0000000000000000000000e+00;
        // x^3 : +0xa.aaaaaaaaaae8p-6
        const double sinh_c3=+1.6666666666667007135061e-01;
        // x^5 : +0x8.8888885894acp-10
        const double sinh_c5=+8.3333333224301781028132e-03;
        // x^7 : +0xd.00fadd88c3468p-16
        const double sinh_c7=+1.9842266695762160997642e-04;

        static const double cs[]= {
            // sinh_c13, sinh_c11, sinh_c9,
            sinh_c7, sinh_c5, sinh_c3
        };
        vf_type xs2=x*x;
        vf_type ym=horner(xs2, cs)*xs2;
        ym= x + x*ym;
        yh = _T::sel(x_lt_ln_2, ym, yh);
    }
    yh = copysign(yh, xc);
    return yh;
#else
    return hyperbolic_k<hyperbolic_func::c_sinh>(xc);
#endif
}
...
cosh_k(arg_t<vf_type> xc)
{
    ...
    vmf_type k_gt_ln2h_and_lt_36=_T::vmi_to_vmf(k <36);
    if (any_of(k_gt_ln2h_and_lt_36)) {
        yl *= sc.f0();
        yl *= sc.f1();
    ...
    if (any_of(x_lt_ln_2)) {
        ...
        const double sinh_c13=+1.6926226991841124453737e-10;
        static const double cs[]= {
            sinh_c13, sinh_c11, sinh_c9, sinh_c7, sinh_c5, sinh_c3
        };
        vf_type xs2=x*x;
        vf_type ym=horner(xs2, cs)*xs2;
        ym= x + x*ym;
    ...
    return yh;
#else
    return hyperbolic_k<hyperbolic_func::c_cosh>(xc);
#endif
}
...
tanh_k(arg_t<vf_type> xc)
{
    ...
    vf_type tanh_x=1.0;
    // x^ : +0xb.17217f7d1cf78p-9
    const double _ln2_32=+2.1660849392498290194586e-02;
    vmf_type x_tiny = xa <= _ln2_32;
    if (any_of(x_tiny)) {
    ...
    vmf_type x_small=(xa > _ln2_32) & (xa<fc::tanh_one());
    if (any_of(x_small)) {
        ...
        vf_type ex, exl;
#if 0
        ex=__exp_tbl_k(xrh, xrl, idx, &exl);
#else
        // [-1.0830424726009368896484375e-2, 1.0830424726009368896484375e-2] : | p - f | <= 2^-62.71875
        // coefficients for exp generated by sollya
        // x^1 : +0x8p-3
        ...
        vf_type p= horner2(xrh, x2, ci);
        vf_type eh = xrh + (xrl+x2*p);
        d_ops::add12(ex, exl, th, tl + th*eh);
#endif
        ...
        tanh_x = _T::sel(x_small, tanh_h, tanh_x);
    }
    tanh_x=copysign(tanh_x, xc);
    return tanh_x;
#else
    return hyperbolic_k<hyperbolic_func::c_tanh>(xc);
#endif
}
