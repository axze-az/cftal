...
namespace cftal {
    namespace math {
        ...
        struct elem_func_core<double, _T> {
            ...
            // arguments are the reduced xrh, xrl in
            // [-0.5, 0.5], and the arguments
            // kf and k as argument for __scale_exp_k
            // calculates 2^(xrh+xrl)*2^k - 1 if exp_m1 is true,
            // 2^(xrh+xrl)*2^k otherwise
            static
            vf_type
            __exp2_k(arg_t<vf_type> xrh, arg_t<vf_type> xrl,
                     arg_t<vf_type> kf, arg_t<vi2_type> k,
                     bool exp_m1);
...
exp_k(arg_t<vf_type> xc, bool exp_m1)
{
    ...
    vf_type y= __exp_k(xrh, xrl, kf, k, exp_m1);
    return y;
}
         ...
         bool exp_m1)
{
    ...
    y = impl::poly(xrh, y,
                   ...
                   exp2_c2);
#if 1
    vf_type yee=M_LN2*xrl + (M_LN2*M_LN2)*xrh*xrl;
    vf_type ye;
    y = y*xrh;
    ...
    if (exp_m1 == false) {
        ...
        // x small, required for handling of subnormal numbers
        using fc=func_constants<double>;
        y = _T::sel((abs(xrh) <= fc::max_denormal()/M_LN2) & (kf==0.0),
                    xrh*M_LN2, y);
    }
#else
    if (exp_m1 == false) {
...
exp2_k(arg_t<vf_type> x, bool exp_m1)
{
    ...
    vf_type y=__exp_k(xrh, xrl, kf, k2, exp_m1);
    return y;
#else
    vf_type xrh, xrl;
    d_ops::add12(xrh, xrl, x, -kf);
    return __exp2_k(xrh, xrl, kf, k2, exp_m1);
#endif
}
...
exp10_k(arg_t<vf_type> x, bool exp_m1)
{
    ...
    return y;
#else
   // [-0.15051500499248504638671875, 0.15051500499248504638671875] : | p - f | <= 2^-57.765625
    // coefficients for exp10 generated by sollya
    // x^0 : +0x8p-3
    const double exp10_c0=+1.0000000000000000000000e+00;
    // x^1 : +0x9.35d8dddaaa8bp-2
    const double exp10_c1=+2.3025850929940459010936e+00;
    // x^2 : +0xa.9a92639e75358p-2
    const double exp10_c2=+2.6509490552392001028181e+00;
    // x^3 : +0x8.2382c8ef15bb8p-2
    const double exp10_c3=+2.0346785922933423584880e+00;
    // x^4 : +0x9.5ebb04fec67ap-3
    const double exp10_c4=+1.1712551489115954339582e+00;
    // x^5 : +0x8.a14ffe907b328p-4
    const double exp10_c5=+5.3938292922221064973343e-01;
    // x^6 : +0xd.3f6b8448f212p-6
    const double exp10_c6=+2.0699584883166821658307e-01;
    // x^7 : +0x8.b726f937f1b28p-7
    const double exp10_c7=+6.8089362801300248873027e-02;
    // x^8 : +0xa.08b51d263beb8p-9
    const double exp10_c8=+1.9597682770033344329397e-02;
    // x^9 : +0xa.44c96ab25cf98p-11
    const double exp10_c9=+5.0140128790148062384868e-03;
    // x^10 : +0x9.7627bed3a5c6p-13
    const double exp10_c10=+1.1549736287761206907088e-03;
    // x^11 : +0xf.c728d1006e91p-16
    const double exp10_c11=+2.4075266355947597682677e-04;
    // x^12 : +0xa.51197af71cf9p-18
    const double exp10_c12=+3.9355449863543489675589e-05;
    // x^ : +0x9.35d8dddaaa8bp-2
    const double log10=+2.3025850929940459010936e+00;
    // x^ : +0xa.9a92639e75348p-1
    const double log10sqr=+5.3018981104783984292794e+00;
    vf_type xx=xr*xr;
    vf_type i=impl::poly(xx,
                         exp10_c11,
                         exp10_c9,
    ...
    vf_type yee= log10*cr + (log10sqr*xr*cr);
    if (exp_m1 == false) {
        vf_type ye;
        ...
        impl::eft_poly_si(y, ye, xr, y, ye, exp10_c0);
        // ye += yee;
        // 2^kf = 2*2^s ; s = kf/2
        vf_type scale = __scale_exp_k(vf_type(0.5), kf, k2);
        // e^x-1 = 2*(y * 2^s - 0.5 * 2^s)
        ...
        // x small, required for handling of subnormal numbers
        // using fc=func_constants<double>;
        y = _T::sel(abs(x) <= 0x1p-64/log10, x*log10, y);
    }
    return y;
#endif
}
