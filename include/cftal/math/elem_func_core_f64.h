...
rsqrt_k(arg_t<vf_type> x)
{
    ...
    // [0.25, 1] : | p - f | <= 2^-9.58984375
    // coefficients for rsqrt generated by sollya
    // x^0 : +0xf.615fb59a5f22p-2
    const double rsqrt_c0=+3.8450916648360209393331e+00;
    // x^1 : -0xc.727cb024d54ep+0
    const double rsqrt_c1=-1.2447215088837133123434e+01;
    // x^2 : +0xd.ce87f74a8f6e8p+1
    const double rsqrt_c2=+2.7613524352461229938172e+01;
    // x^3 : -0x8.b3e78955bdadp+2
    const double rsqrt_c3=-3.4811006864291542228784e+01;
    // x^4 : +0xb.7312d58fe2f7p+1
    const double rsqrt_c4=+2.2899012274987335047172e+01;
...
__pow_log_k2(arg_t<vf_type> xc)
{
    ...
    // [3.4694469519536141888238489627838134765625e-18, 0.17157287895679473876953125] : | p - f | <= 2^-75.0625
    // coefficients for log generated by sollya
    // x^2 h: +0xa.aaaaaaaaaaaa8p-4
    const vf_type log_c2h=+6.6666666666666662965923e-01;
    // x^2 l: +0xa.aa3975c81e9bp-58
    const vf_type log_c2l=+3.7001441047630636651916e-17;
    // x^4 : +0xc.cccccccccccdp-5
    const vf_type log_c4=+4.0000000000000002220446e-01;
    // x^6 : +0x9.2492492491b88p-5
    const vf_type log_c6=+2.8571428571426965570268e-01;
    // x^8 : +0xe.38e38e3a38ebp-6
    const vf_type log_c8=+2.2222222222707327388136e-01;
    // x^10 : +0xb.a2e8b95a74348p-6
    const vf_type log_c10=+1.8181818104659733248418e-01;
    // x^12 : +0x9.d89dd6a6c0cp-6
    const vf_type log_c12=+1.5384622537659353724848e-01;
    // x^14 : +0x8.8877a1854d94p-6
    const vf_type log_c14=+1.3332930348880123183619e-01;
    // x^16 : +0xf.13938f52a5d2p-7
    const vf_type log_c16=+1.1778492448983840512611e-01;
    // x^18 : +0xd.1fe49ff6b0928p-7
    const vf_type log_c18=+1.0253579912876327473459e-01;
    // x^20 : +0xf.aa8b8ffb4bb08p-7
    const vf_type log_c20=+1.2239212541916578980139e-01;
    // vf_type w = z*z;
    vf_type y= horner(zh,
                      log_c20,
                      log_c18,
                      ...
                      log_c10);
    vf_type ye;
    horner_comp_quick(y, ye, zh, y, log_c8, log_c6, log_c4);
    d_ops::mul22(y, ye, zh, zl, y, ye);
    d_ops::add22cond(y, ye, y, ye, log_c2h, log_c2l);
    d_ops::mul22(y, ye, zh, zl, y, ye);
    using ctbl=impl::d_real_constants<d_real<double>, double>;
    vf_type kf = _T::cvt_i_to_f(_T::vi2_odd_to_vi(k));

    dvf_type log_x= kf* dvf_type(ctbl::m_ln2);
    log_x += dvf_type(ds*dhfsq);
    log_x += dvf_type(ds*dvf_type(y, ye));
    // log_x -= dhfsq;
    // log_x += df;
    log_x += (df -dhfsq);
    return log_x;
#else
    vf_type s = dvf_type(df/(vf_type(2.0)+df)).h();
    vf_type z = s*s;
    // [3.4694469519536141888238489627838134765625e-18, 0.17157287895679473876953125] : | p - f | <= 2^-64.625
    // coefficients for log generated by sollya
    // x^2 : +0xa.aaaaaaaaaaaa8p-4
    ...
    vf_type y= horner(z,
                      ...
                      log_c10);
    vf_type ye;
    horner_comp_quick(y, ye, z, y, log_c8, log_c6, log_c4, log_c2);
    d_ops::mul122(y, ye, z, y, ye);
    using ctbl=impl::d_real_constants<d_real<double>, double>;
    vf_type kf = _T::cvt_i_to_f(_T::vi2_odd_to_vi(k));

    dvf_type log_x= kf* dvf_type(ctbl::m_ln2);
    log_x += dvf_type(s*dhfsq);
    log_x += dvf_type(s*dvf_type(y, ye));
    // log_x -= dhfsq;
    // log_x += df;
    log_x += (df -dhfsq);
    return log_x;
