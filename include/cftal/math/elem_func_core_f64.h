...
#if 0
    using ctbl = impl::d_real_constants<d_real<double>, double>;
    vf_type xl, xh=d_ops::two_prod(xr, ctbl::m_ln2.h(), xl);
    // exp(x) = 1 + 2x/(2-x+x^2*P(x^2)
    //                    x [x - x^2 * P]
    //        = 1 + x + ------------------
    //                    2 - [x - x^2 * P]
...
exp10_k(arg_t<vf_type> x)
{
    ...
    // [-0.15051500499248504638671875, 0.15051500499248504638671875] : | p - f | <= 2^-66.6875
    // coefficients for exp10 generated by sollya
    // x^0 h: +0xd.e5bd8a937287p-4
    const vf_type exp10_c0h=+8.6858896380650363333586e-01;
    // x^0 l: +0xc.aae2ed0ac849p-59
    const vf_type exp10_c0l=+2.1974653199758126067721e-17;
    // x^2 : +0xc.47cbd278e3638p-5
    const vf_type exp10_c2=+3.8376418216567426133778e-01;
    // x^4 : -0x8.ae695edce2258p-8
    const vf_type exp10_c4=-3.3911309871549187577866e-02;
    // x^6 : +0x8.c46184931fc78p-11
    const vf_type exp10_c6=+4.2808168967808498880934e-03;
    // x^8 : -0x9.4be52bd5da828p-14
    const vf_type exp10_c8=-5.6741123202433462000688e-04;
    // x^10 : +0x9.f4dbd81af429p-17
    const vf_type exp10_c10=+7.5961910672635025694802e-05;
    // x^12 : -0xa.872d68a206dd8p-20
    const vf_type exp10_c12=-1.0040318138232877070277e-05;
    ...
    vf_type P = impl::poly(xx,
                           ...
                           exp10_c2) *xx;
    vf_type dx= (hi-xr);
    vf_type xl = dx-kf * ctbl::m_ld2_cw[1];
    vf_type q = P -xr + exp10_c0l + exp10_c0h;
    // f(x) := 10^(r+c);
    // f(x) ~ 10^r + log(10) 10^r c + ..
    // 10^r ~ 1 + log(10) r
    // f(x) ~ 10^r + (1+log(10)*r)*log(10)*c
    //      = 10^r + log(10)*c + log(10)*log(10)*r*c
    vf_type yee1=log10sqr*xr*xl;
    vf_type yee= log10*xl + yee1;
    vf_type y=0;
    y += yee;
    y += (2*xr)/q;
    y += 1;
...
{
    // return x+y;
    vf_type lnx = log_k(x, log_func::c_log_e);
