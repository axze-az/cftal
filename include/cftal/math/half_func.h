...
inline
typename cftal::math::half_func<float, _T>::vf_type
cftal::math::half_func<float, _T>::__half_log_k(arg_t<vf_type> xc)
{
...
cftal::math::half_func<float, _T>::__half_log_poly_k(arg_t<vf_type> xc)
{
    vf_type xr, kf;
    base_type::__reduce_log_arg(xr, kf, xc);
    // [-0.292893230915069580078125, 0.41421353816986083984375] : | p - f | <= 2^-26.53125
    ...
    // x^2 : -0xf.ffffcp-5f
    constexpr
    const float log_f16_c2=-4.9999988079e-01f;
    // x^3 : +0xa.aab7ep-5f
    constexpr
    const float log_f16_c3=+3.3333963156e-01f;
    // x^4 : -0x8.0028ap-5f
    constexpr
    const float log_f16_c4=-2.5001937151e-01f;
    // x^5 : +0xc.c688dp-6f
    constexpr
    const float log_f16_c5=+1.9961757958e-01f;
    // x^6 : -0xa.9a6cbp-6f
    constexpr
    const float log_f16_c6=-1.6567532718e-01f;
    // x^7 : +0x9.8c432p-6f
    constexpr
    const float log_f16_c7=+1.4918592572e-01f;
    // x^8 : -0x9.2ac2bp-6f
    constexpr
    const float log_f16_c8=-1.4323489368e-01f;
    // x^9 : +0xb.32ee6p-7f
    constexpr
    const float log_f16_c9=+8.7491795421e-02f;
...
{
    using ctbl = impl::d_real_constants<d_real<float>, float>;
    vf_type x = __half_log_k(d)*ctbl::m_1_ln2[0];
    const vf_type pinf(_T::pinf());
    const vf_type ninf(_T::ninf());
    x = _T::sel(isinf(d), pinf, x);
    // if (d < 0) x = NAN;
