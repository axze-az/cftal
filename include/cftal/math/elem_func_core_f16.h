...
__log_tbl_k12(arg_t<vf_type> xc)
{
    ...
    // x^3 : +0xa.acp-5_f16
    constexpr
    const f16_t log_hp_c3=+3.33496e-01_f16;
    // x^4 : -0x8.02p-5_f16
    constexpr
    const f16_t log_hp_c4=-2.50244e-01_f16;
    static_assert(log_hp_c1 == 1.0_f16);
    static_assert(log_hp_c2 ==-0.5_f16);
    vf_type r2=r*r;
    vf_type p=horner(r, log_hp_c4, log_hp_c3);
#else
    // [-1.5625e-2, 1.5625e-2] : | p - f | <= 2^-25.9453125
    // coefficients for log_hp generated by sollya
    // x^1 : +0x8p-3_f16
...
#if 0
    vf_type ckf, ckfl;
    d_ops::mul12(ckf, ckfl, kf, ctbl::m_ln2[0]);
    vf_type t1, t;
    d_ops::add12(t1, t, ckf, log_c_h);
    vf_type t2= t1 + r;
    vf_type l1= (kf* ctbl::m_ln2[1] + ckfl) + t + log_c_l;
#else
    vf_type t1, t;
...
pow_k(arg_t<vf_type> x, arg_t<vf_type> y)
{
...
#else
    const f16_t scale_down=0.5_f16;
    d_ops::mul122(ylnx[0], ylnx[1], y*scale_down,
                  lnx[0]*scale_down, lnx[1]*scale_down);
    const f16_t scale_up=4.0_f16;
    ylnx[0] *= scale_up;
    ylnx[1] *= scale_up;
#endif
...
#if 0
    vf_type ll;
    vf_type res=__exp_tbl_k(xrh, xrl, idx, ki, &ll);
    res += ll;
#else
    ...
    res = _T::sel((d > exp_hi_inf) |
                  ((d == exp_hi_inf) & (ylnx[1] >= 0.0_f16)),
                  _T::pinf(), res);
