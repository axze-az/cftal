        ...
        arg_t<vf_type> x)
{
    ...
    // x^0 : +0x8p-3
    constexpr
    const float exp_c0=+1.0000000000000000000000e+00;
    // x^1 : +0x8p-3
    constexpr
    const float exp_c1=+1.0000000000000000000000e+00;
    // x^2 : +0x8.000001d0d64b8p-4
    constexpr
    const float exp_c2=+5.0000000676426992196610e-01;
    // x^3 : +0xa.aaaaa21b31aep-6
    constexpr
    const float exp_c3=+1.6666665869413577194535e-01;
    // x^4 : +0xa.aaa46ec513648p-8
    constexpr
    const float exp_c4=+4.1666295092978318514998e-02;
    // x^5 : +0x8.88938475a673p-10
    constexpr
    const float exp_c5=+8.3334970089822928140944e-03;
    // x^6 : +0xb.6c679f8263a88p-13
    constexpr
    const float exp_c6=+1.3944648636194554528617e-03;
    // x^7 : +0xc.f8452631b28p-16
    constexpr
    const float exp_c7=+1.9790352070606781476059e-04;
...
__exp_tbl_k(arg_t<vf_type> xr, arg_t<vi_type> idx)
{
    ...
    constexpr
    const float exp_c2=+5.0000366357879488798943e-01;
    // x^3 : +0xa.aaac332fabb6p-6
    constexpr
    const float exp_c3=+1.6666703222898193725854e-01;
    static_assert(exp_c1==1.0, "oops");
    vf_type x2=xr*xr;
...
__log_k(arg_t<vf_type> xc)
{
    ...
    constexpr
    const float log_c1=+9.9999999995908828154256e-01;
    // x^2 : -0xf.ffffc1217c6bp-5
    constexpr
    const float log_c2=-4.9999988289699837107349e-01;
    // x^3 : +0xa.aaaa83416f8a8p-5
    constexpr
    const float log_c3=+3.3333325992451651176296e-01;
    // x^4 : -0x8.002145c45f9fp-5
    constexpr
    const float log_c4=-2.5001586557754140738297e-01;
    // x^5 : +0xc.cd1f5e066525p-6
    constexpr
    const float log_c5=+2.0001968557478938537386e-01;
    // x^6 : -0xa.a1387ca90d9cp-6
    constexpr
    const float log_c6=-1.6609012769569075906873e-01;
    // x^7 : +0x9.138d3cd84f5p-6
    constexpr
    const float log_c7=+1.4181834165468831798762e-01;
    // x^8 : -0x8.79cf021e5c7e8p-6
    constexpr
    const float log_c8=-1.3243460852522295767564e-01;
    // x^9 : +0x8.424ce7ea5f5b8p-6
    constexpr
    const float log_c9=+1.2904665611266860625328e-01;
    // x^10 : -0x9.c14a282383dep-7
    constexpr
    const float log_c10=-7.6211232756604851967808e-02;
    constexpr
    static const float ci[]={
        log_c10,
...
__log_k12(arg_t<vf_type> xc)
{
    ...
    constexpr
    const float log_c2=-5.0000000284167267494695e-01;
    // x^3 : +0xa.aaaaa36a67dbp-5
    constexpr
    const float log_c3=+3.3333331982725711295501e-01;
    // x^4 : -0xf.fffd6786b4f1p-6
    constexpr
    const float log_c4=-2.4999938116054837289326e-01;
    // x^5 : +0xc.ccd0d882e603p-6
    constexpr
    const float log_c5=+2.0000096458133514998323e-01;
    // x^6 : -0xa.ab4437fb0c538p-6
    constexpr
    const float log_c6=-1.6670327631817835611905e-01;
    // x^7 : +0x9.24919a40f1ebp-6
    constexpr
    const float log_c7=+1.4285697997891472210696e-01;
    // x^8 : -0xf.e3600a9c1c398p-7
    constexpr
    const float log_c8=-1.2412643926674764294216e-01;
    // x^9 : +0xe.1b644643830a8p-7
    constexpr
    const float log_c9=+1.1021092825742358212171e-01;
    // x^10 : -0xd.ea0bac440641p-7
    constexpr
    const float log_c10=-1.0870500480091219164258e-01;
    // x^11 : +0xd.bb064f4d63e08p-7
    constexpr
    const float log_c11=+1.0727003929312585450706e-01;
    // x^12 : -0xf.58521b5ab5cd8p-8
    constexpr
    const float log_c12=-5.9941417387299376906551e-02;
