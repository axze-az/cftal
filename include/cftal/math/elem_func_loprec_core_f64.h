...
sinh_cosh_k(arg_t<vf_type> xc)
{
    ...
    if (_F == hyperbolic_func::c_sinh) {
        ...
        if (__likely(_T::any_of_vmf(x_tiny))) {
            // [0, 0.17328679561614990234375] : | p - f | <= 2^-34.984375
            // coefficients for sinh generated by sollya
            // x^1 : +0x8.00000005c246p-3
            constexpr
            const double sinh_c1=+1.0000000001676054850464e+00;
            // x^3 : +0xa.aaaa3ea78b248p-6
            constexpr
            const double sinh_c3=+1.6666656607246668753142e-01;
            // x^5 : +0x8.8ae0565bda89p-10
            constexpr
            const double sinh_c5=+8.3422711095076131082582e-03;
            vf_type xs2=x*x;
            vf_type yt=horner(xs2, sinh_c5, sinh_c3, sinh_c1)*x;
            res = yt;
        }
    }
