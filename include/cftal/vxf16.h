...
//
    ...
    struct mem< vec<f16_t, 1> > {
        ...
        static
        void store(f16_t* p, const vec<f16_t, 1>& v, ssize_t n=1) {
            using v_t = vec<mf_f16_t, 1>;
            mem<v_t>::store(reinterpret_cast<mf_f16_t*>(p), v(), n);
        }
    };
    ...
    struct mem< vec<f16_t, _N> > {
        static
        vec<f16_t, _N> load(const f16_t* p, ssize_t n=_N) {
            using v_t= vec<mf_f16_t, _N>;
            v_t r(mem<v_t>::load(reinterpret_cast<const mf_f16_t*>(p), n));
            return vec<f16_t, _N>::cvt_from_rep(r);
        }
        static
        void store(f16_t* p, const vec<f16_t, _N>& v, ssize_t n=_N) {
            using v_t = vec<mf_f16_t, _N>;
            mem<v_t>::store(reinterpret_cast<mf_f16_t*>(p), v(), n);
        }
    };
    ...
    template <size_t _N>
    struct is_vec_specialized<vec<f16_t, _N> >
        : public is_vec_specialized<vec<mf_f16_t, _N> > {
    };
    ...
    namespace impl {
        ...
        template <>
        class variable_vec_lookup_table<f16_t, int32_t, 4> {
            vec<int32_t, 4> _idx;
        public:
            ...
            fromp(const f16_t* tbl) const {
                ...
                vec<mf_f16_t, 4> r(low_half(rlh));
                return vec<f16_t, 4>::cvt_from_rep(r);
            }
        };

        template <>
        class variable_vec_lookup_table<f16_t, int32_t, 8> {
            vec<int32_t, 8> _idx;
        public:
