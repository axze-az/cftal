...
inline
cftal::vec<cftal::int16_t, 8>
cftal::mem<cftal::vec<int16_t, 8> >::load(const int16_t* p, ssize_t s)
{
    vec<int16_t, 8> v;
    if (s >= 8) {
        ...
        if (s == 5) {
            ...
            const int32_t* p32=reinterpret_cast<const int32_t*>(p+4);
...
inline
void
cftal::mem<cftal::vec<int16_t, 8> >::
store(int16_t* p, const vec<int16_t, 8>& v, ssize_t s)
{
    if (s >= 8) {
        ...
        _mm_storeu_si64(p, v());
        if (s > 4) {
            vec<int16_t, 8> t=permute<4, 5, 6, 7, 0, 1, 2, 3>(v);
            if ( s == 5) {
                _mm_storeu_si16(p+4, t());
            } else if (s>=6) {
                _mm_storeu_si32(p+4, t());
                if (s>=7) {
                    int16_t e6=extract<6>(v);
                    p[6] = e6;
                }
            }
        }
...
inline
cftal::vec<int16_t, 8>
cftal::expand_mask<cftal::vec<int16_t, 8> >::
...
#else
    return _mm_maskz_mov_epi16(m(), on_true());
#endif
}
...
#else
    return _mm_maskz_mov_epi16(_knot_mask8(m()), on_false());
#endif
}
