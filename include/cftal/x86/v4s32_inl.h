...
inline
cftal::vec<cftal::int32_t, 4>
cftal::mem<cftal::vec<int32_t, 4> >::load(const int32_t* p, ssize_t s)
{
    __m128i v;
    if (s>=4) {
...
inline
void
cftal::mem<cftal::vec<int32_t, 4> >::
store(int32_t* p, const vec<int32_t, 4>& v, ssize_t s)
{
    if (s>=4) {
        _mm_storeu_si128(reinterpret_cast<__m128i*>(p), v());
    } else if (s==3) {
        _mm_storeu_si64(p, v());
        vec<int32_t, 4> t=permute<2, 3, 0, 1>(v);
