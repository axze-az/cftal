...
#else
                v16u16 ta(a ^ full_type(sign_s16_msk::v.u16()));
                v16u16 tb(b ^ full_type(sign_s16_msk::v.u16()));
...
inline
cftal::vec<cftal::uint16_t, 16>
cftal::mem<cftal::vec<uint16_t, 16> >::load(const uint16_t* p, ssize_t s)
{
    vec<uint16_t, 16> v;
    if (s >= 16) {
        v = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(p));
    } else if (s==1) {
        v = _mm256_castsi128_si256(_mm_loadu_si16(p));
    } else if (s>=2) {
        auto lh=mem<vec<uint16_t, 8> >::load(p, s);
        ssize_t sh= s>=8 ? s-8 : 0;
        auto hh=mem<vec<uint16_t, 8> >::load(p+8, sh);
        v = vec<uint16_t, 16>(lh, hh);
    } else {
...
inline
void
cftal::mem<cftal::vec<uint16_t, 16> >::
store(uint16_t* p, const vec<uint16_t, 16>& v, ssize_t s)
{
    if (s>=16) {
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(p), v());
    } else if (s>0) {
        mem<vec<uint16_t, 8> >::store(p, low_half(v), s);
        if (s>8) {
            mem<vec<uint16_t, 8> >::store(p+8, high_half(v), s-8);
        }
    }
}
...
cftal::compress_mask(const vec<uint16_t, 16>::mask_type& v)
{
...
inline
cftal::vec<uint16_t, 8>
cftal::odd_elements(const vec<uint16_t, 16>& a)
{
