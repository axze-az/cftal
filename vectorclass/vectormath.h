...
static inline Vec4f cexp (Vec4f const & x) {   // complex exponential function
    float xx[4], yy[4], ee[2];
    x.store(xx);
    ee[0] = expf(xx[0]);  ee[1] = expf(xx[2]);
    yy[0] = cosf(xx[1])*ee[0];  yy[1] = sinf(xx[1])*ee[0];  
    yy[2] = cosf(xx[3])*ee[1];  yy[3] = sinf(xx[3])*ee[1];
    return Vec4f(yy[0],yy[1],yy[2],yy[3]);
}
static inline Vec2d cexp (Vec2d const & x) {   // complex exponential function
    double xx[2], yy[2], ee;
    x.store(xx);
    ee = exp(xx[0]);
    yy[0] = cos(xx[1])*ee;  yy[1] = sin(xx[1])*ee;  
    return Vec2d(yy[0],yy[1]);
}
