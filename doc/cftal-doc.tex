...
\texttt{include/cftal/math/elem\_func.h}
...
or double double variable is floating point expansion
\[
    (x_h, x_l) = x_h + x_l \, |x_l| < \frac{1}{2} |x_h^{-n}|
\]
where n is the number of mantissa bits in the double variable.
%
%TODO
%
\subsubsection{roots}
...
\subsubsection{argument reduction for exp2(x) and exp2m1(x)}
    ...
    and then scaled
    \[
       (r_h, r_l) = r_0 - k * (LN2_{HI}, LN2_{LO})
    \]
    to allow the calculation of $ 2^x + C $ as $ 2^k * e^{r_h+ rl} + C$.
...
\subsubsection{argument reduction for exp10(x) and expm10m1(x)}
    ...
    \[
       \begin{aligned}
       hi &= x - k \times LG2_{HI} \\
       r_h &= hi - k \times LG2_{LO} \\
       dx & = hi - r_h \\
       r_l &= dx - k \times LG2_{LO}
       \end{aligned}
    \]
    where $LG2_{HI}$ and $LG2_{LO}$ are Cody and Waite constants for
    $log_{10}(2)$
    and the expressions are organized to allow utilization of fma operations.
    This reduced argument is then scaled
    \[
        \begin{aligned}
        (r_h, r_l) &= r_{0h} - k * (LN10_{HI}, LN10_{LO}) \\
        r_l &= r_l + r_{0l} * LN10_{HI}
        \end{aligned}
    \]
    to allow the calculation of $ 10^x + C $ as $ 10^k * e^{r_h+ rl} + C$.
    The constants $LN10_{HI}$ and $LN10_{LO}$ are the two parts of a double double number for $log(10)$.


\subsubsection{approximation of $e^r$ in [$-log(2)/2, log(2)/2$] and scaling}
    ...
    using a error free transformation of the horner schema to achieve
