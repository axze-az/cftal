...
bool x86vec::test::check_popcnt()
{
	...
	std::uint64_t exp_u128;
        ...
        for (int i=0; i<256; i+=16) {
                ...
                __m128i ru16= popcnt_u16(src._v);
                __m128i ru32= popcnt_u32(src._v);
                __m128i ru64= popcnt_u64(src._v);
		std::uint64_t ru128 = popcnt_u128(src._v);
...
bool x86vec::test::check_bitrev()
{
	...
	std::uint64_t exp_u64[2];
        ...
        for (int i=0; i<256; i+=16) {
                ...
                __m128i ru64= bitrev_u64(src._v);
                r &= chk(extract_u8<0>(ru8), exp_u8[0], src._u8[0],
			 "bitrev_u8");
                r &= chk(extract_u8<1>(ru8), exp_u8[1], src._u8[1],
			 "bitrev_u8");
